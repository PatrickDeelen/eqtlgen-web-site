...
In case of any issues when running this cookbook please contact Urmo Võsa (urmo.vosa at gmail.com) or Robert Warmerdam (c.a.warmerdam@umcg.nl).
...
-   If the pipeline crashes (e.g. due to wall time), you can just resubmit the same script after any required changes are implemented.
...
-   Nextflow and Singularity by default create cache folders in your home directory. This may unexpectedly fill your home.
    ...
    scheme from the [setup](setup) chapter and run the pipelines from within the pipeline directories,
    these evaluate to `eQTLGen_phase2/singularitycache` and `eQTLGen_phase2/nextflowcache`.
    Be aware that these might be changed from your default if you've sourced the startup script.
    Nextflow and Singularity cache within the environments wherein workflow tasks are be executed can also be set;
    Please see the config files under `// Set singularity and nextflow cachedirs for task execution environments`
...
- We encourage the use of a VCF dataset for WGS data. This enables the DataQC step to perform variant level WGS-specific quality control.
- A VCF dataset can either be split by chromosome (by using a globbing pattern surrounded with quotes), or it can be a single VCF.

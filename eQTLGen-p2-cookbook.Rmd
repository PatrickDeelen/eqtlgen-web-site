...
- We require either an unimputed PLINK dataset. [.bed](https://www.cog-genomics.org/plink/1.9/formats#bed)/[.bim](https://www.cog-genomics.org/plink/1.9/formats#bim)/[.fam](https://www.cog-genomics.org/plink/1.9/formats#fam) or an unimputed VCF dataset. Can be in hg19/GRCh37 or hg38/GRCh38.
- WGS datasets are expected to have some sample-level quality control previouslydone
...
If the genotype data is available for \<5,000 samples, the pipeline uses only those samples which are avilable.
...
❗ While continuing with the next step, now it is also good time to send the `Report_DataQc_[your cohort name].html` to Urmo Võsa (urmo.vosa at gmail.com) and Robert Warmerdam (c.a.warmerdam at umcg.nl). This way we can have a initial look to the quality of the data and let you know if something seems un-optimal.
